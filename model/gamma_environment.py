        ...
        if sequence == []:
            ...
            if (episode < 25000) :
                ...
                for _ in range(output_sequence_length) :
                    ...
                    val = (val/20.0) * (kind_cars - 1)
                    if ((val > kind_cars) or (val == kind_cars)) :
                        val = torch.tensor(kind_cars) - 0.5
                    self.output_sequence.append(int(torch.floor(val).item()))
                self.fill_gamma(random = True)
                
                for _ in range(self.length+self.input_window_length+1) :
                    val = distribution.sample()
                    val = (val/20.0) * (kind_cars - 1)
                    if ((val > kind_cars) or (val == kind_cars)) :
                        val = torch.tensor(kind_cars) - 0.5
                    self.input_sequence.append(int(torch.floor(val).item()))
